...
export default function Faces() {
  ...
  const filteredFaces = mockFaces.filter((face) => {
    ...
    return (
      face.playerName.toLowerCase().includes(q) ||
      face.team.toLowerCase().includes(q)
    );
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {filteredFaces.length} faces shown
          </p>
        </div>
      ...
      {/* Grid View */}
      {viewMode === "grid" && filteredFaces.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {filteredFaces.map((face) => (
            <div
              key={face.id}
              className="card-gaming p-4 group cursor-pointer hover:-translate-y-1 hover:shadow-lg transition-all"
            >
      ...
      {/* List View */}
      {viewMode === "list" && filteredFaces.length > 0 && (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border">
                {filteredFaces.map((face) => (
                  <tr key={face.id} className="table-row-gaming">
