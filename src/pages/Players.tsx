...
export default function Players() {
  // Get players from the CORRECT store
  const storeData = useEditBinStore((state) => state.data);
  const players: Player[] = storeData?.players || [];
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center mb-4">
              Import a decrypted EDIT00000000 file to view players.
            </p>
            <Button asChild>
              ...
              <TableBody>
                {paginatedPlayers.map((player, index) => (
                  <TableRow key={`${player.id}-${index}`}>
                    ...
                    <TableCell className="text-muted-foreground">{player.shirtName || '-'}</TableCell>
